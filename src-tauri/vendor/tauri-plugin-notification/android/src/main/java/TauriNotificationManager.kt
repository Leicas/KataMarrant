...
// KataMarrant fork: dedicated deep-link action for the notification content /
// action-button PendingIntents. Upstream reused ACTION_MAIN + CATEGORY_LAUNCHER
// (the home-launcher intent), which makes a cold (process-dead) tap resolve to a
// launcher/Recents task-switch and land the user on the home screen instead of
// the app. buildIntent sets this action; NotificationPlugin.onIntent gates on it.
const val NOTIFICATION_OPEN_ACTION = "app.tauri.notification.OPEN"
...
class TauriNotificationManager(
  ...
  private fun buildIntent(notification: Notification, action: String?): Intent {
    ...
    // KataMarrant fork: deep-link into our own activity instead of firing the
    // home-launcher intent. Drop ACTION_MAIN + CATEGORY_LAUNCHER (root cause of
    // the cold tap landing on the home screen). Add FLAG_ACTIVITY_NEW_TASK,
    // required because the content PendingIntent fires from the headless
    // TimedNotificationPublisher BroadcastReceiver on a cold start. Keep
    // SINGLE_TOP so a warm task delivers via onNewIntent. CLEAR_TOP is dropped:
    // our single-activity app has nothing above to clear, and it was implicated
    // in a cold-task finish/relaunch bounce.
    intent.removeCategory(Intent.CATEGORY_LAUNCHER)
    intent.action = NOTIFICATION_OPEN_ACTION

...
class NotificationStorage(private val context: Context, private val jsonMapper: ObjectMapper) {
  ...
  fun writeActionGroup(actions: List<ActionType>) {
    for (type in actions) {
      ...
      // KataMarrant fork: index the per-action keys by their position
      // (id0/id1/…) so they match what getActionGroup reads back. Upstream
      // 2.3.3 wrote "id$typeId" for EVERY action (same key, last write wins)
      // while getActionGroup reads "id0".."id{count-1}", so action buttons
      // always came back with empty id/title and never rendered/routed.
      for ((i, action) in type.actions.withIndex()) {

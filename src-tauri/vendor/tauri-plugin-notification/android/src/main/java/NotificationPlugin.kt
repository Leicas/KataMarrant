...
class NotificationPlugin(private val activity: Activity): Plugin(activity) {
  ...
  fun onIntent(intent: Intent) {
    // KataMarrant fork: gate on the dedicated deep-link action that buildIntent
    // now sets (was ACTION_MAIN). Cold-launch taps arrive via load() ->
    // activity.intent; warm taps via onNewIntent.
    if (NOTIFICATION_OPEN_ACTION != intent.action) {
      return
    }
